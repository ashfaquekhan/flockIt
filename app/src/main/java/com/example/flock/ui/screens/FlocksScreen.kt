...
private fun CreateFlockDialog(
    ...
    val showDatePicker = {
        val current = try { LocalDate.parse(startDate) } catch (e: Exception) { LocalDate.now() }
        DatePickerDialog(
            context,
                    ...
                    OutlinedTextField(
                        ...
                        trailingIcon = {
                            IconButton(onClick = showDatePicker) {
                                Icon(Icons.Default.Egg, contentDescription = "Pick date", tint = BrandEmerald)
                            }
                        },
                        modifier = Modifier.weight(1.4f).clickable { showDatePicker() }
                    )
                    OutlinedTextField(
                        value = startTime,
                        onValueChange = {},
                        ...
                        trailingIcon = {
                            IconButton(onClick = showTimePicker) {
                                Icon(Icons.Default.Settings, contentDescription = "Pick time", tint = BrandEmerald)
                            }
                        },
                        modifier = Modifier.weight(1.1f).clickable { showTimePicker() }
                    )
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
